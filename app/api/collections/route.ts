...
export async function POST(request: NextRequest) {
  try {
    ...
    if (validatedData.isSmartCollection && rules && rules.length > 0) {
      ...
      collection = await collectionService.createCollection(collectionData, session.user.id);

      // Add garments if provided
      if (garmentIds && garmentIds.length > 0) {
        await collectionService.addGarmentsToCollection(collection.id, garmentIds, session.user.id);

        // Fetch the updated collection with garments
        collection = await collectionService.getCollectionById(collection.id, session.user.id);
      }
    }
